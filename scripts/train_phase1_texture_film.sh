...
# 单层 FiLM：冻结 E5；正确文本与错配文本组分别运行、分别保存。
# DRY_RUN=1 bash scripts/train_phase1_texture_film.sh 可查看完整命令。
PROJECT_ROOT="${PROJECT_ROOT:-/share/home/u2515283058/Mymodel}"
DATASETS_ROOT="${DATASETS_ROOT:-/share/home/u2515283058/datasets}"
OUTPUT_BASE="${OUTPUT_BASE:-${PROJECT_ROOT}/output}"
FILM_TEXT_MODE="${FILM_TEXT_MODE:-matched}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${OUTPUT_BASE}/phase1_film_${FILM_TEXT_MODE}}"
...
start_step=0
resume_args=()
if [[ -n "${FILM_RESUME_CKPT:-}" ]]; then
  # 只恢复模型权重，优化器和调度器重新初始化。
  resume_args=(--resume_from_checkpoint "${FILM_RESUME_CKPT}")
  start_step=-1
fi
cmd=(
  ...
  --film_hidden_dim "${FILM_HIDDEN_DIM:-128}" --film_lr "${FILM_LR:-5e-5}" --film_text_mode "${FILM_TEXT_MODE}"
  --learning_rate "${FILM_LR:-5e-5}" --seed "${TRAIN_SEED:-42}"
  ...
  --max_train_samples "${MAX_TRAIN_SAMPLES:-0}" --max_train_steps "${MAX_TRAIN_STEPS:-1000}"
  ...
  --wandb_run_name "film_${FILM_TEXT_MODE}"
)
printf '%q ' "${cmd[@]}"
printf '\n'
if [[ "${DRY_RUN:-0}" != "1" ]]; then
