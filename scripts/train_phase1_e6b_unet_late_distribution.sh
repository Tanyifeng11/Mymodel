...
NUM_TRAIN_EPOCHS="${E6B_NUM_TRAIN_EPOCHS:-1}"
MAX_TRAIN_STEPS="${E6B_MAX_TRAIN_STEPS:--1}"
...
UNET_LATE_LR="${UNET_LATE_LR:-2e-6}"
UNET_LATE_BLOCKS="${UNET_LATE_BLOCKS:-2,3}"
TRAIN_UNET_OUTPUT_LAYER="${TRAIN_UNET_OUTPUT_LAYER:-1}"
FREEZE_TCPM_FOR_DISTRIBUTION="${FREEZE_TCPM_FOR_DISTRIBUTION:-1}"
...
WANDB_RUN_NAME="${WANDB_RUN_NAME:-phase1_e6b_unet_late_dist_e1}"
...
if [[ -n "${RESUME_CKPT}" && ! -e "${RESUME_CKPT}" ]]; then
  echo "[ERROR] E6B_RESUME_CKPT does not exist: ${RESUME_CKPT}" >&2
  exit 1
fi
...
CMD=(
  ...
  --train_unet_late_for_distribution 1
  --unet_late_lr "${UNET_LATE_LR}"
  --unet_late_blocks "${UNET_LATE_BLOCKS}"
  --train_unet_output_layer "${TRAIN_UNET_OUTPUT_LAYER}"
  --freeze_tcpm_for_distribution "${FREEZE_TCPM_FOR_DISTRIBUTION}"
...
echo "============================================"
echo "Phase 1 E6b UNet late distribution tuning"
echo "BASE_CKPT=${BASE_CKPT}"
echo "RESUME_CKPT=${RESUME_CKPT}"
echo "TEXTURE_ADAPTER_CKPT=${TEXTURE_ADAPTER_CKPT}"
echo "OUTPUT_DIR=${OUTPUT_DIR}"
echo "UNET_LATE_BLOCKS=${UNET_LATE_BLOCKS}, TRAIN_UNET_OUTPUT_LAYER=${TRAIN_UNET_OUTPUT_LAYER}"
echo "UNET_LATE_LR=${UNET_LATE_LR}, FREEZE_TCPM_FOR_DISTRIBUTION=${FREEZE_TCPM_FOR_DISTRIBUTION}"
