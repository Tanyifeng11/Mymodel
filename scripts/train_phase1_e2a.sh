...
# 阶段 2A：从 E1 checkpoint 初始化，加入区域纹理约束、边界约束和泄露约束。

# 显卡数量：E2A 默认单卡，直接改这里，或运行时用 NUM_GPUS=1 bash scripts/train_phase1_e2a.sh 覆盖。
NUM_GPUS="${NUM_GPUS:-1}"
...
OUTPUT_DIR="${E2A_OUTPUT_DIR:-${OUTPUT_BASE}/phase1_e2a_region_e5}"
...
TORCH_SHOW_CPP_STACKTRACES="${TORCH_SHOW_CPP_STACKTRACES:-0}"

TRAIN_BATCH_SIZE="${GAM_TRAIN_BATCH_SIZE:-1}"
GRADIENT_ACCUMULATION_STEPS="${GAM_GRADIENT_ACCUMULATION_STEPS:-2}"
NUM_TRAIN_EPOCHS="${E2A_NUM_TRAIN_EPOCHS:-${GAM_NUM_TRAIN_EPOCHS:-5}}"
MAX_TRAIN_STEPS="${E2A_MAX_TRAIN_STEPS:-${GAM_MAX_TRAIN_STEPS:--1}}"
CHECKPOINTING_EPOCHS="${GAM_CHECKPOINTING_EPOCHS:-1}"
LEARNING_RATE="${E2A_LEARNING_RATE:-${GAM_LEARNING_RATE:-5e-5}}"
...
WANDB_RUN_NAME="${E2A_WANDB_RUN_NAME:-phase1_e2a_region_e5}"
...
if [[ "${SMOKE_TEST}" == "1" ]]; then
  ...
  LAMBDA_REGION_TEXTURE="${SMOKE_LAMBDA_REGION_TEXTURE:-0.1}"
  LAMBDA_BOUNDARY="${SMOKE_LAMBDA_BOUNDARY:-0.05}"
  LAMBDA_LEAK="${SMOKE_LAMBDA_LEAK:-0.1}"
...
if [[ -z "${GAM_INIT_CKPT}" ]]; then
  GAM_INIT_CKPT="$(find_latest_gam_ckpt "${E1_OUTPUT_DIR}")"
fi

if [[ "${FORCE_TRAIN:-0}" != "1" ]]; then
  EXISTING_CKPT="$(find_latest_gam_ckpt "${OUTPUT_DIR}")"
  if [[ -n "${EXISTING_CKPT}" ]]; then
    echo "[SKIP] E2A checkpoint already exists: ${EXISTING_CKPT}"
    exit 0
  fi
fi
...
if [[ ! -f "${GAM_INIT_CKPT}" ]]; then
  echo "[ERROR] GAM_INIT_CKPT does not exist: ${GAM_INIT_CKPT}" >&2
  echo "[ERROR] Run E1 first, or pass GAM_INIT_CKPT=/path/to/e1/joint_model.pt" >&2
  exit 1
fi
...
echo "============================================"
echo "Phase 2A E1 + region constraints training"
...
echo "LAMBDA_REGION_TEXTURE=${LAMBDA_REGION_TEXTURE}, LAMBDA_BOUNDARY=${LAMBDA_BOUNDARY}, LAMBDA_LEAK=${LAMBDA_LEAK}, REGION_KERNEL_SIZE=${REGION_KERNEL_SIZE}"
echo "CUDA_DEBUG=${CUDA_DEBUG}, CUDA_LAUNCH_BLOCKING=${CUDA_LAUNCH_BLOCKING}, TORCH_SHOW_CPP_STACKTRACES=${TORCH_SHOW_CPP_STACKTRACES}, SMOKE_TEST=${SMOKE_TEST}"
