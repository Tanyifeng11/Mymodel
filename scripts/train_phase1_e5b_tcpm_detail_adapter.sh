...
BF_TRAIN_ROOT="${BF_ROOT}/training"
...
OUTPUT_DIR="${E5B_OUTPUT_DIR:-${OUTPUT_BASE}/phase1_e5b_tcpm_detail_adapter_e1}"
...
NUM_TRAIN_EPOCHS="${E5B_NUM_TRAIN_EPOCHS:-1}"
MAX_TRAIN_STEPS="${E5B_MAX_TRAIN_STEPS:--1}"
CHECKPOINTING_EPOCHS="${GAM_CHECKPOINTING_EPOCHS:-1}"
NUM_WARMUP_STEPS="${GAM_NUM_WARMUP_STEPS:-200}"
...
DETAIL_TEXTURE_ADAPTER_LR="${DETAIL_TEXTURE_ADAPTER_LR:-2e-6}"
TRAIN_DETAIL_TEXTURE_GATE="${TRAIN_DETAIL_TEXTURE_GATE:-0}"

LAMBDA_STYLE="${LAMBDA_STYLE:-0.7}"
STYLE_LOSS_TYPE="${STYLE_LOSS_TYPE:-gram}"
LAMBDA_EDGE="${LAMBDA_EDGE:-0.05}"
LAMBDA_TEXTURE_COLOR="${LAMBDA_TEXTURE_COLOR:-0.1}"
LAMBDA_REGION_TEXTURE="${LAMBDA_REGION_TEXTURE:-0.05}"
LAMBDA_BOUNDARY="${LAMBDA_BOUNDARY:-0.045}"
LAMBDA_LEAK="${LAMBDA_LEAK:-0.08}"
LAMBDA_REGION_COLOR_LAB="${LAMBDA_REGION_COLOR_LAB:-0.03}"
...
WANDB_RUN_NAME="${WANDB_RUN_NAME:-phase1_e5b_tcpm_detail_adapter_e1}"
...
CMD=(
  ...
  --train_detail_texture_adapter 1
  --detail_texture_adapter_lr "${DETAIL_TEXTURE_ADAPTER_LR}"
  --train_detail_texture_gate "${TRAIN_DETAIL_TEXTURE_GATE}"
...
echo "============================================"
echo "Phase 1 E5b TCPM-lite + detail texture adapter training"
echo "BASE_CKPT=${BASE_CKPT}"
echo "TEXTURE_ADAPTER_CKPT=${TEXTURE_ADAPTER_CKPT}"
echo "OUTPUT_DIR=${OUTPUT_DIR}"
echo "TCPM_LR=${TCPM_LR}, TCPM_SCALE_LR=${TCPM_SCALE_LR}"
echo "DETAIL_TEXTURE_ADAPTER_LR=${DETAIL_TEXTURE_ADAPTER_LR}, TRAIN_DETAIL_TEXTURE_GATE=${TRAIN_DETAIL_TEXTURE_GATE}"
echo "LOSSES style=${LAMBDA_STYLE}, tex_color=${LAMBDA_TEXTURE_COLOR}, region_tex=${LAMBDA_REGION_TEXTURE}, region_lab=${LAMBDA_REGION_COLOR_LAB}, boundary=${LAMBDA_BOUNDARY}, leak=${LAMBDA_LEAK}"
