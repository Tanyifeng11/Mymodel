...
OUTPUT_DIR="${E4B_OUTPUT_DIR:-${OUTPUT_BASE}/phase1_e4b_boundary_fid_safe_e1}"
...
NUM_TRAIN_EPOCHS="${E4B_NUM_TRAIN_EPOCHS:-1}"
MAX_TRAIN_STEPS="${E4B_MAX_TRAIN_STEPS:--1}"
CHECKPOINTING_EPOCHS="${GAM_CHECKPOINTING_EPOCHS:-1}"
LEARNING_RATE="${E4B_LEARNING_RATE:-5e-6}"
PALETTE_MLP_LR="${PALETTE_MLP_LR:-3e-5}"
NUM_WARMUP_STEPS="${GAM_NUM_WARMUP_STEPS:-200}"
MAX_GRAD_NORM="${GAM_MAX_GRAD_NORM:-1.0}"
DATALOADER_NUM_WORKERS="${DATALOADER_NUM_WORKERS:-1}"

BF_NUM_TOKENS="${BF_NUM_TOKENS:-16}"
NUM_PALETTE_TOKENS="${NUM_PALETTE_TOKENS:-4}"
...
LAMBDA_BOUNDARY="${LAMBDA_BOUNDARY:-0.06}"
...
BALANCED_GATE_REG_WEIGHT="${BALANCED_GATE_REG_WEIGHT:-3e-4}"

REPORT_TO="${REPORT_TO:-wandb}"
WANDB_PROJECT="${WANDB_PROJECT:-Mymodel}"
WANDB_RUN_NAME="${E4B_WANDB_RUN_NAME:-phase1_e4b_boundary_fid_safe_e1}"
...
CMD=(
  ...
  --balanced_gate_reg_weight "${BALANCED_GATE_REG_WEIGHT}"
...
echo "============================================"
echo "Phase E4B boundary/FID-safe balanced gate training"
echo "E4A_CKPT=${E4A_CKPT}"
echo "TEXTURE_ADAPTER_CKPT=${TEXTURE_ADAPTER_CKPT}"
echo "OUTPUT_DIR=${OUTPUT_DIR}"
echo "LEARNING_RATE=${LEARNING_RATE}, PALETTE_MLP_LR=${PALETTE_MLP_LR}"
echo "BALANCED_GATE_REG_WEIGHT=${BALANCED_GATE_REG_WEIGHT}"
echo "LAMBDA_BOUNDARY=${LAMBDA_BOUNDARY}, LAMBDA_TEXTURE_COLOR=${LAMBDA_TEXTURE_COLOR}, LAMBDA_REGION_TEXTURE=${LAMBDA_REGION_TEXTURE}, LAMBDA_REGION_COLOR_LAB=${LAMBDA_REGION_COLOR_LAB}, LAMBDA_LEAK=${LAMBDA_LEAK}"
echo "============================================"
printf '%q ' "${CMD[@]}"
echo
