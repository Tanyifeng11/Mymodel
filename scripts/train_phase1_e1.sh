...
# 阶段 1：训练 E1 分组版。使用纹理 token 条件，并启用层级分组。

# 显卡数量：E1 默认单卡，直接改这里，或运行时用 NUM_GPUS=1 bash scripts/train_phase1_e1.sh 覆盖。
NUM_GPUS="${NUM_GPUS:-1}"
...
OUTPUT_DIR="${E1_OUTPUT_DIR:-${OUTPUT_BASE}/phase1_e1_grouped_e5}"
...
LEARNING_RATE="${E1_LEARNING_RATE:-${GAM_LEARNING_RATE:-5e-5}}"
...
WANDB_RUN_NAME="${E1_WANDB_RUN_NAME:-phase1_e1_grouped_e5}"
...
if [[ "${FORCE_TRAIN:-0}" != "1" ]]; then
  ...
  if [[ -n "${EXISTING_CKPT}" ]]; then
    echo "[SKIP] E1 checkpoint already exists: ${EXISTING_CKPT}"
    exit 0
  fi
fi
...
echo "============================================"
echo "Phase 1 E1 grouped training"
