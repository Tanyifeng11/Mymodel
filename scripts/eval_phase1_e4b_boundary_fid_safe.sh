...
EVAL_BASE="${EVAL_BASE:-${PROJECT_ROOT}/eval_outputs/phase1_full_500_e4b_boundary_fid_safe}"
...
if [[ "${METRICS_ONLY:-0}" != "1" && ! -f "${E4B_CKPT}" ]]; then
  echo "[ERROR] E4B_CKPT does not exist: ${E4B_CKPT}" >&2
  echo "[ERROR] Train it first with: bash scripts/train_phase1_e4b_boundary_fid_safe.sh" >&2
  exit 1
fi
...
benchmark_cmd=(
  ...
  --run_name e4b_boundary_fid_safe
)
...
echo "============================================"
echo "Phase E4B boundary/FID-safe evaluation"
echo "E4B_CKPT=${E4B_CKPT}"
echo "TEXTURE_ADAPTER_CKPT=${TEXTURE_ADAPTER_CKPT}"
echo "EVAL_BASE=${EVAL_BASE}"
echo "REPORT_DIR=${REPORT_DIR}"
echo "EVAL_NUM_SAMPLES=${EVAL_NUM_SAMPLES}, EVAL_DEVICE=${EVAL_DEVICE}"
echo "SAVE_BALANCED_GATE_TRACE=${SAVE_BALANCED_GATE_TRACE}"
echo "============================================"
...
report_cmd=(
  ...
  --experiment_names e4b_boundary_fid_safe
  --device "${REPORT_DEVICE}"
  --clip_model_path "${CLIP_MODEL_PATH}"
)

echo "=== Building E4B report ==="
printf '%q ' "${report_cmd[@]}"
echo
...
echo "============================================"
echo "E4B evaluation done."
echo "Eval output:   ${EVAL_BASE}/e4b_boundary_fid_safe"
