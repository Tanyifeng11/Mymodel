#!/bin/bash
#SBATCH -J E16_B_spatial
#SBATCH -p gpu
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=40G
#SBATCH --gres=gpu:1
#SBATCH -o log_e16_b_%j.log
#SBATCH -e log_e16_b_%j.err
# E16-B：把 resampler 的 64 个全局 query 换成 8x8 二维空间 query，其余超参与 A1 完全一致。
#
# 目的：区分「token 容量不足」和「query 缺少空间归纳偏置」。A1（64 全局 query）没有改善，
# 因此这里保持 token 数、步数、lr、数据顺序、随机种子不变，只把 query 变成具有明确位置
# 职责的 8x8 网格，并给 query 与 CNN stage token 各加一份二维正弦位置编码。
set -eo pipefail
if [[ "${DRY_RUN:-0}" != 1 ]]; then
...
train_arm() {  # $1=输出子目录 $2=token 数
 ...
 run python train_texture_adapter.py \
  ...
  --bf_num_tokens "${tokens}" --bf_query_layout spatial \
...
train_arm e16_b_spatial64 64

# 复用 E15-D1/D2：token 数由权重决定，64 token 检查点直接走同一套诊断。
for PAIR in "b_spatial64:e16_b_spatial64"; do
 TAG="${PAIR%%:*}"; ARM="${PAIR##*:}"
