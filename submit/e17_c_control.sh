...
DIRECT="${E17_ROOT}/direct_train/checkpoint-final/joint_model.pt"
...
python -m tools.e17_encoder_control \
  ...
  --output "${E17_ROOT}/c_encoder_positive" --device cuda:0
python -m tools.e17_probe \
  ...
  --checkpoint "${E17_ROOT}/c_encoder_positive/pytorch_model.bin" \
  ...
  --output "${E17_ROOT}/c_probe" --device cuda:0
python -m tools.e17_direct_response \
  --manifest "${PROJECT_ROOT}/data/train_bf_texture.json" \
  --data-root "${DATA_ROOT:-/share/home/u2515283058/datasets/BF/training}" \
  --base-checkpoint "${BASE_BF}" \
  --direct-checkpoint "${E17_ROOT}/c_encoder_positive/joint_model.pt" \
  --base-model "${PROJECT_ROOT}/models/stable-diffusion-v1-5" \
  --clip-model "${PROJECT_ROOT}/models/clip" \
  --output "${E17_ROOT}/c_response" --device cuda:0
python -m tools.e15_d5_generation \
  --manifest "${PROJECT_ROOT}/data/train_bf_texture.json" \
  --data-root "${DATA_ROOT:-/share/home/u2515283058/datasets/BF/training}" \
  --checkpoint "${E17_ROOT}/c_encoder_positive/joint_model.pt" \
  ...
  --output "${E17_ROOT}/unused_layers_c" \
  --output-reference "${E17_ROOT}/generation_c"
