...
#SBATCH --mem=32G
#SBATCH --gres=gpu:1
#SBATCH -o log_e14_real_orientation_%j.log
#SBATCH -e log_e14_real_orientation_%j.err
set -eo pipefail
if [[ "${DRY_RUN:-0}" != 1 ]]; then
...
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/eval_outputs/e14_real_orientation/${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
...
if [[ "${DRY_RUN:-0}" != 1 ]]; then
  [[ -f "${INPUTS_ROOT}/feature_labels.csv" ]] || { echo "先解压输入包到 ${INPUTS_ROOT}" >&2; exit 1; }
...
# 分类在下载结果后本地执行，服务器无需安装 sklearn。
echo "提取完成后下载 ${EVAL_ROOT}，运行 tools.e14_real_orientation evaluate"
