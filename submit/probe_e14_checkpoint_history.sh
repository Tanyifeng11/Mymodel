...
#SBATCH --mem=32G
#SBATCH --gres=gpu:1
#SBATCH -o log_e14_history_%j.log
#SBATCH -e log_e14_history_%j.err
set -eo pipefail
if [[ "${DRY_RUN:-0}" != 1 ]]; then
...
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/eval_outputs/e14_checkpoint_history/${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
...
echo "输出目录：${EVAL_ROOT}；下载后本地执行 tools.e14_checkpoint_history evaluate --root <目录>"
