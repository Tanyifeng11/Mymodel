#!/bin/bash
# �ύ��sbatch submit/compare_sptg.sh
# Ԥ����DRY_RUN=1 bash submit/compare_sptg.sh
# ð�̣�NUM_SAMPLES=2 sbatch submit/compare_sptg.sh
#SBATCH -J E5_SPTG
#SBATCH -p gpu
#SBATCH -N 1
#SBATCH --ntasks=1
...
#SBATCH --mem=32G
#SBATCH --gres=gpu:1
#SBATCH -o log_sptg_%j.log
#SBATCH -e log_sptg_%j.err
...
SOURCE_RUN="${SOURCE_RUN:-${PROJECT_ROOT}/eval_outputs/full_condition_probe_6/112711}"
DATASET_JSON="${DATASET_JSON:-${SOURCE_RUN}/dataset.json}"
# ���� E12 �ѹ̶��� validation ����˳��ֻ��ȡǰ N �š�
SPLIT_PATH="${SPLIT_PATH:-${SOURCE_RUN}/fixed_split.json}"
...
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/eval_outputs/sptg_6/${SLURM_JOB_ID:-$(date +%Y%m%d_%H%M%S)}}"
...
for sid in 2 5 14 18 22 23; do
  ...
  for variant in baseline strong_texture sptg_rho05 sptg_rho10; do
    run python tools/run_fixed_benchmark.py "${common[@]}" --run_name e5 \
      --sample_id_start "${sid}" --sample_id_end "$((sid + 1))" \
      --sptg_mode "${variant}" --output_dir "${EVAL_ROOT}/${sample_name}/${variant}"
  done
done
run python tools/report_sptg.py --run-dir "${EVAL_ROOT}" --source-run "${SOURCE_RUN}"
