#!/bin/bash
#SBATCH -J E14_balance
#SBATCH -p gpu
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --gres=gpu:1
#SBATCH -o log_e14_balance_%j.log
#SBATCH -e log_e14_balance_%j.err
set -eo pipefail
if [[ "${DRY_RUN:-0}" != 1 ]]; then
...
CMD=(python -m tools.e14_source_balance
 ...
 --output "${EVAL_ROOT:-${PROJECT_ROOT}/eval_outputs/e14_source_balance/${SLURM_JOB_ID:-local}}")
printf '%q ' "${CMD[@]}"; printf '\n'
