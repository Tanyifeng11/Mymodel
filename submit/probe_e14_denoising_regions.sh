#!/bin/bash
#SBATCH -J E14_regions
#SBATCH -p gpu
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=40G
#SBATCH --gres=gpu:1
#SBATCH -o log_e14_regions_%j.log
#SBATCH -e log_e14_regions_%j.err
set -eo pipefail
if [[ "${DRY_RUN:-0}" != 1 ]]; then
...
CMD=(python -m tools.e14_matched_denoising --regions
 ...
 --output "${EVAL_ROOT:-${PROJECT_ROOT}/eval_outputs/e14_denoising_regions/${SLURM_JOB_ID:-local}}")
printf '%q ' "${CMD[@]}"; printf '\n'
