...
from tools.e17_probe import train_network
...
def main():
    ...
    result = {"protocol": {"probe_dir": str(root), "clip_tokens": clip_length,
              ...
              "tasks": {}}
    for task in ("orientation", "pattern"):
        entry = report["tasks"][task]
        label = "axis" if task == "orientation" else "pattern"
        classes = np.array(sorted(entry["classes"]))
        scores = {source: {model: [] for model in ("mlp", "attention")} for source in sources}
        for fold, split in enumerate(entry["splits"]):
            ...
            labels = {name: np.array([rows[i][label] for i in indices])
